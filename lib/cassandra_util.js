...
var cassandraClient = require('./cassandra');
var backupModel = require('../models/backup/backup_model');
var BackupFields = backupModel.BackupFields;
var ObjectState = require('../constants/ObjectStates');
var logger = require('../logger/winston_logger');
//var queryTimeLogger = logger.specialFile('cassandra_query.log');
var MasterProcessActions = require('../constants/MasterProcessActions');

var BACKUP_TABLE_NAME = 'backup';
var TAG = 'cassandra_util: ';

var performanceMonitor = require('./performance_monitor');

var _this = this;

var TABLES_NOT_BACKUP = ['backup', 'action_affect_suggestion', 'question_exercise_by_session'];
var TABLES_BACKUP = ['tweet', 'files', 'new_control_line'];
...
var generateFields = function (model, instance) {
    var result = '(';

    for(var field in model) {
        if(instance[model[field]] != undefined) {
            result += model[field] + ',';
        }
    }
...
var generateValues = function (model, instance) {
    var result = '(';
...
var generateParams = function (model, instance) {
    var result = [];
    for(var field in model) {
        if(instance[model[field]] != undefined) {
            result.push(instance[model[field]]);
        }
    }
...
var generateAssignments = function (model, instance) {
    var assignments = '';
    for(var field in model) {
        if(instance[model[field]] != undefined) {
...
var generateAssignmentCounter = function (model, instance) {
    var assignments = '';
    for(var field in model) {
        if(instance[model[field]] != undefined) {
            assignments += model[field] + ' = ' + model[field] + ' + ?,';
        }
    }
...
var generateDecreaseCounter = function (model, instance) {
    var assignments = '';
    for(var field in model) {
        if(instance[model[field]] != undefined) {
            assignments += model[field] + ' = ' + model[field] + ' - ?,';
...
var cloneInstance = function (model, instance) {
    var newInstance = {};
    for(var field in model) {
        if(instance[model[field]] != undefined) {
            newInstance[model[field]] = instance[model[field]];
        }
    }
    return newInstance;
};
...
exports.generateCounterUpdate = function (model, instance) {
    var assignments = '';
    for(var field in model) {
        if(instance[model[field]] != undefined) {
            assignments += model[field] + ' = ' + model[field] + ' + ?,';
