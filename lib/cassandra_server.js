...
if(process.env) {
    ...
    options.user = process.env.user || process.env.userName || options.user;
    options.password = process.env.password || process.env.pass || options.password;
    options.keyspace = process.env.keyspace || options.keyspace;
    options.id = process.env.id || options.id;
}
 ...
 */
var cassServer = new IPCServer(options.id);
cassServer.on('started', function () {
    if(process.send) {
